...
import Svg, { Circle, Defs, Ellipse, Line, LinearGradient, Path, Polygon, RadialGradient, Rect as SvgRect, Stop, Text as SvgText } from 'react-native-svg';
...
const BAR_NEON_COLORS = [
  { id: 'pink',     color: '#e040fb', label: '핑크',   labelEn: 'Pink'   },
  { id: 'cyan',     color: '#00e0ff', label: '사이언', labelEn: 'Cyan'   },
  { id: 'amber',    color: '#ffb830', label: '앰버',   labelEn: 'Amber'  },
  { id: 'green',    color: '#00e880', label: '그린',   labelEn: 'Green'  },
  { id: 'red',      color: '#ff4060', label: '레드',   labelEn: 'Red'    },
  { id: 'lavender', color: '#9060ff', label: '라벤더', labelEn: 'Violet' },
] as const;
...
const BAR_WALL_THEMES = [
  { id: 'wood',   label: '우드',     labelEn: 'Wood',   darkBg: '#1c1a14', lightBg: '#e4dfd6' },
  { id: 'brick',  label: '브릭',     labelEn: 'Brick',  darkBg: '#1e1210', lightBg: '#ede0d8' },
  { id: 'slate',  label: '슬레이트', labelEn: 'Slate',  darkBg: '#141820', lightBg: '#dde0e8' },
  { id: 'forest', label: '포레스트', labelEn: 'Forest', darkBg: '#111814', lightBg: '#d8e4d8' },
] as const;
...
const T_EN = {
  ...
  selectIngr:  'SELECT INGREDIENTS (MAX 4)',
...
const useMixStore = create<MixStore>((set) => ({
  ...
  addPart: (id) => set((s) => {
    ...
    if (idx > -1) {
      ...
      if (s.selections.length >= 4) return s;
...
function HomeScreenEcho() {
  const { lang, theme, bgmPlaying, homeCabH, inventoryItems } = useAppStore();
  const C      = useColors();
  const isDark = useIsDark();
  const styles = useStyles();
  const t      = TEXTS[lang];
        ...
        {homeCabH > 0 && shelfPlanks.map((plankY, i) => (
          <React.Fragment key={i}>
            ...
            <View style={{ position: 'absolute', top: plankY + 3, left: SHELF_MX, right: SHELF_MX, height: PLANK_H + 6,
              backgroundColor: C.shelf, borderTopWidth: 2, borderTopColor: C.shelfEdge }} />
          </React.Fragment>
        ))}
      </View>
    </View>
...
function BarWall() {
  ...
  const signColor  = isDark ? barNeonColor : C.accent;
...
// ─────────────────────────────────────────────────────────────────
// SHELF BOTTLE  (tap + long-press drag, lives inside a ScrollView)
// ─────────────────────────────────────────────────────────────────
function ShelfBottle({ item, index, onTap, isAnyFocused, isFocused,
                       isDragging, isAnyDragging,
                       onDragStart, onDragEnd,
                       scrollViewRef, scrollXRef }: {
  ...
  scrollXRef: { current: number };
}) {
  const imgH   = Math.round(BOTTLE_H * BOTTLE_VSCALE[index % BOTTLE_VSCALE.length]);
  ...
  }, [isDragging, isAnyDragging, isFocused, isAnyFocused, opacityAnim, scaleAnim]);
  ...
  const panResponder = useRef(PanResponder.create({
    ...
    onPanResponderGrant: () => {
      ...
      gestureStartScrollX.current = scrollXRef.current;
    ...
    onPanResponderMove: (_, gs) => {
      if (isDraggingRef.current) {
        ...
        scrollViewRef.current?.scrollTo({ x: Math.max(0, gestureStartScrollX.current - gs.dx), animated: false });
      } else if (Math.abs(gs.dx) > 8 && Math.abs(gs.dx) > Math.abs(gs.dy) * 1.5) {
    ...
    onPanResponderRelease: (_, gs) => {
      if (timerRef.current) { clearTimeout(timerRef.current); timerRef.current = null; }
      const wasD = isDraggingRef.current;
      const wasS = isScrollingRef.current;
      isDraggingRef.current  = false;
      isScrollingRef.current = false;
      if (wasD) {
        Animated.spring(dragAnim, { toValue: { x: 0, y: 0 }, useNativeDriver: true, friction: 10, tension: 80 }).start();
        onDragEnd(item.id, Math.round(gs.dx / SLOT_W));
      } else if (!wasS && Math.abs(gs.dx) < 8 && Math.abs(gs.dy) < 8) {
        onTap(item);
      }
    },
    ...
    // Prevent SlidingTabContainer from stealing this gesture mid-swipe (would cause screen transition)
  ...
  return (
    <Animated.View
      ...
      style={{
        ...
        zIndex: isDragging ? 100 : 1, elevation: isDragging ? 20 : 1,
        transform: [{ translateX: dragAnim.x }, { translateY: dragAnim.y }, { scale: scaleAnim }],
      }}
    >
      <Animated.View style={{
        position: 'absolute',
        ...
        <ExpoImage
          source={{ uri: imgUri }}
          style={{ width: BOTTLE_W, height: imgH }}
          contentFit="contain"
          transition={200}
        />
      </Animated.View>
...
function WoodPlank({ shelfW }: { shelfW: number }) {
  const C      = useColors();
  const isDark = useIsDark();
  ...
  return (
    <View style={{
      ...
      <View style={{ height: PLANK_H, backgroundColor: C.shelf,
        borderTopWidth: 2, borderTopColor: C.shelfEdge, overflow: 'hidden' }}>
        <Svg style={StyleSheet.absoluteFill} width={shelfW} height={PLANK_H}>
...
// ─────────────────────────────────────────────────────────────────
// SHELF ROW  (margins + horizontal-only scroll + wood plank flush with bottles)
// ─────────────────────────────────────────────────────────────────
function ShelfRow({ plankY, items, focusedId, onTapBottle, draggingId, onDragStart, onDragEnd }: {
  ...
  const isDark  = useIsDark();
  const shelfW  = SCREEN_W - SHELF_MX * 2;
  const scrollW = Math.max(shelfW, items.length * SLOT_W);

  const scrollViewRef = useRef<ScrollView | null>(null);
  const scrollXRef    = useRef(0);
  const { inventoryItems } = useAppStore();

  // Claim horizontal gestures that start inside this shelf row so they can't
  // bubble up to SlidingTabContainer and trigger a screen transition.
  // onShouldBlockNativeResponder:false lets the native ScrollView still scroll
  // even while the JS responder is claimed here.
  const blockTabSwipe = useRef(PanResponder.create({
  ...
  return (
    <View
      ...
      {...blockTabSwipe.panHandlers}
    >
      {/* Wood plank: rendered first so bottles appear on top */}
      <View style={{ position: 'absolute', top: BOTTLE_H - SHELF_OVERLAP, left: 0, right: 0 }}>
        <WoodPlank shelfW={shelfW} />
      </View>
      {/* LED strip: bright core fading to transparent — no hard bottom edge */}
      <Svg
      ...
      {/* Bottles: rendered last so they appear above the plank */}
      <ScrollView
        ref={scrollViewRef}
        horizontal
      ...
      >
        {items.map((item) => (
          <ShelfBottle
            key={item.id}
            item={item}
            index={inventoryItems.indexOf(item)}
            onTap={onTapBottle}
...
function BarCounterDecor() {
  const isDark = useIsDark();

  // Marble colors
  const marbleBase = isDark ? '#161210' : '#eae5db';
  const topBase    = isDark ? '#1c1916' : '#f0ebe0';
...
function WallCDPlayer() {
  ...
  useEffect(() => {
    if (bgmPlaying) {
      ...
      glow.start();
      return () => {
        spin.stop(); spinAnim.setValue(0);
        glow.stop(); glowAnim.setValue(0);
      };
    } else {
      spinAnim.setValue(0);
      glowAnim.setValue(0);
  ...
  // Body occupies the left 60px; right 80px is cord exit space (exits screen right)
  const BSPC = 80;            // horizontal space right of body for cord
  const BW = 60, BH = 60;
  const W  = BW + BSPC;      // total width = 140
  const H  = BH + 82;        // total height = 142
  const BX = 0;              // body left edge = 0
  const CX = BX + BW / 2;   // CD centre x = 30
  const CY = BH / 2;         // CD centre y = 30
  const CR = 21, CDR = CR - 1;

  const bodyFill   = isDark ? '#2c2c3a' : '#f5f3ef';
  const bodyStroke = isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.10)';
  ...
  return (
    <TouchableOpacity onPress={toggleBgm} activeOpacity={0.8} style={{ width: W, height: H }}>
      ...
      {/* Glow halo behind body — pulses amber when playing */}
      <Animated.View
        pointerEvents="none"
        style={{
          position: 'absolute',
          left: BX - 7, top: -7,
          width: BW + 14, height: BH + 14,
          borderRadius: 17,
          opacity: glowAnim,
          borderWidth: 1,
          borderColor: 'rgba(248,192,64,0.55)',
          shadowColor: '#f8c040',
          shadowRadius: 18,
          shadowOpacity: 1,
          shadowOffset: { width: 0, height: 0 },
      ...
      <Svg width={W} height={BH} style={{ position: 'absolute', top: 0 }} pointerEvents="none">
        <SvgRect x={BX} y={0} width={BW} height={BH} rx={10} fill={bodyFill} />
        <SvgRect x={BX} y={0} width={BW} height={BH} rx={10} fill="none"
          stroke={bodyStroke} strokeWidth="1" />
        <Line x1={BX + 8} y1={1.5} x2={BX + BW - 8} y2={1.5}
          stroke={isDark ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.80)'}
          strokeWidth="1" strokeLinecap="round" />
        <Circle cx={CX} cy={CY} r={CR + 2} fill={isDark ? '#1a1a26' : '#e0ddd8'} />
        {[-10, -5, 0, 5, 10].map(dy => (
          <Circle key={`l${dy}`} cx={BX + 7} cy={CY + dy} r={1.1} fill={dotFill} />
        ))}
        {[-10, -5, 0, 5, 10].map(dy => (
          <Circle key={`r${dy}`} cx={BX + BW - 7} cy={CY + dy} r={1.1} fill={dotFill} />
        ))}
      </Svg>
      ...
      <Animated.View
        ...
        style={{
          position: 'absolute',
          left: CX - CDR, top: CY - CDR,
          width: CDR * 2, height: CDR * 2,
      ...
      >
        <Svg width={CDR * 2} height={CDR * 2}>
          ...
          <Circle cx={CDR} cy={CDR} r={CDR - 7}  fill="#d87828" />
          <Circle cx={CDR} cy={CDR} r={CDR - 9}  fill="#e49030" />
          <Circle cx={CDR} cy={CDR} r={CDR - 11} fill="#f0a838" />
          <Circle cx={CDR} cy={CDR} r={CDR - 13} fill="#f8c040" />
          <Circle cx={CDR} cy={CDR} r={7}   fill="#f0a820" />
          <Circle cx={CDR} cy={CDR} r={5}   fill="#e09010" />
          <Circle cx={CDR} cy={CDR} r={2.5} fill="#111" />
          <Circle cx={CDR} cy={CDR} r={1}   fill="#555" />
          {/* asymmetric highlights — make rotation clearly visible */}
          <Circle cx={CDR - 8}  cy={CDR - 9}  r={3.5} fill="rgba(255,255,255,0.18)" />
          <Circle cx={CDR + 11} cy={CDR - 5}  r={2.2} fill="rgba(255,240,160,0.28)" />
          <Circle cx={CDR - 3}  cy={CDR + 13} r={1.8} fill="rgba(255,200,80,0.22)" />
        </Svg>
      </Animated.View>

      {/* CD glow ring — pulses amber when playing */}
      <Animated.View
        pointerEvents="none"
        style={{
          position: 'absolute',
          left: CX - CR - 4, top: CY - CR - 4,
          width: (CR + 4) * 2, height: (CR + 4) * 2,
          borderRadius: CR + 4,
          opacity: glowAnim,
          borderWidth: 1.5,
          borderColor: 'rgba(248,192,64,0.65)',
          shadowColor: '#f8c040',
          shadowRadius: 10,
          shadowOpacity: 1,
          shadowOffset: { width: 0, height: 0 },
      ...
      <Svg width={W} height={BH} style={{ position: 'absolute', top: 0 }} pointerEvents="none">
        ...
        <Circle cx={CX} cy={CY} r={CR}
          ...
          strokeWidth="1.2" />
        <Path d={`M ${CX - 13} ${CY - 16} Q ${CX} ${CY - 24} ${CX + 13} ${CY - 16}`}
          fill={isDark ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.55)'} />
      </Svg>

      {/* Layer 4: cord — straight drop, wiggles, drifts right, fades out off screen */}
      <Svg width={W} height={H - BH} style={{ position: 'absolute', top: BH }} pointerEvents="none">
        <Defs>
          <LinearGradient id="crdFade" gradientUnits="userSpaceOnUse"
            x1={CX} y1={0} x2={W - 6} y2={H - BH - 6}>
        ...
        <Path
          d={`M ${CX} 0 L ${CX} 13 Q ${CX - 9} 21 ${CX} 29 Q ${CX + 9} 37 ${CX - 2} 45 Q ${CX + 16} 54 ${CX + 42} 60 Q ${CX + 70} 67 122 73 Q 134 77 140 80`}
          stroke="url(#crdFade)"
          strokeWidth="1.6"
          fill="none"
          strokeLinecap="round"
...
function BottomPanel({ item, t, onClose }: { item: InventoryItem; t: Texts; onClose: () => void }) {
  const { lang, updateInventoryItem, removeInventoryItem } = useAppStore();
  const C        = useColors();
  const styles   = useStyles();
...
function AtmosphereCard({ hour, lang, onBarMode, isActive }: {
  ...
  const cardBg = isDark ? C.surfaceHi : C.surface;
      ...
      <View style={{ padding: 14, paddingLeft: 18, paddingBottom: 14 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
          <Text style={{ fontSize: 10, fontWeight: '800', color: C.textDim, letterSpacing: 1.2, textTransform: 'uppercase', flex: 1 }}>
            {lang === 'ko' ? '오늘 밤의 분위기' : "TONIGHT'S ATMOSPHERE"}
          </Text>
...
function AddSpiritModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  const { lang, addInventoryItem, setAuthModalVisible } = useAppStore();
  const { user } = useAuthStore();
  const C = useColors();

  const requireAuth = (action: () => void) => {
    if (!user) { setAuthModalVisible(true); return; }
    action();
  };
    ...
    const loadIngredients = async () => {
      ...
      try {
        ...
        if (json && json.drinks) {
          ...
          setIngredients(list);
          setFiltered(list.slice(0, 30)); // initial list
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const match = ingredients.filter(i =>
        i.toLowerCase().includes(searchQuery.toLowerCase())
      );
      setFiltered(match.slice(0, 30));
    }
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.65)', justifyContent: 'flex-end' }}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ height: '88%', backgroundColor: C.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, overflow: 'hidden', borderWidth: 1, borderColor: C.border }}>
          <View style={{ height: 4, width: 40, borderRadius: 2, backgroundColor: C.border, alignSelf: 'center', marginTop: 10, marginBottom: 12 }} />
          ...
          <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
            ...
            <TextInput
              ...
              onChangeText={(txt) => {
                setSearchQuery(txt);
                if (selectedApiName && txt !== selectedApiName) {
                  setSelectedApiName(null);
                }
              }}
            />
            ...
            {/* Suggestions list */}
            {!selectedApiName && (
              <View style={{ maxHeight: 150, backgroundColor: C.surfaceHi, borderRadius: 10, marginTop: 4, borderWidth: 1, borderColor: C.border, overflow: 'hidden' }}>
                {loading ? (
                  ...
                  <ScrollView nestedScrollEnabled keyboardShouldPersistTaps="handled">
                    {filtered.map(item => (
                      <TouchableOpacity
                        ...
                        style={{ paddingVertical: 12, paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: C.border }}
                      >
            ...
            {/* Selected Preview & Brand/ABV inputs */}
            {selectedApiName && (
          ...
          {!user && (
            <TouchableOpacity onPress={() => setAuthModalVisible(true)}
              style={{
                ...
                backgroundColor: isDark ? 'rgba(255,184,48,0.10)' : 'rgba(184,120,0,0.08)',
                borderRadius: 12, borderWidth: 1,
                borderColor: isDark ? 'rgba(255,184,48,0.25)' : 'rgba(184,120,0,0.20)',
                paddingHorizontal: 14, paddingVertical: 10,
              }}>
              <Text style={{ fontSize: 15 }}>🔒</Text>
              <Text style={{ flex: 1, fontSize: 12, fontWeight: '700', color: isDark ? '#ffb830' : '#b87800' }}>
                {lang === 'ko' ? '로그인 후 커스텀 설정을 저장할 수 있어요' : 'Sign in to save your customizations'}
              </Text>
              <Text style={{ fontSize: 13, color: isDark ? '#ffb830' : '#b87800', fontWeight: '700' }}>›</Text>
            </TouchableOpacity>
          )}
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', gap: 14, flexWrap: 'wrap' }}>
              {BAR_NEON_COLORS.map(nc => {
                ...
                return (
                  <TouchableOpacity key={nc.id} onPress={() => requireAuth(() => setBarNeonColor(nc.color))}
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row', gap: 12, flexWrap: 'wrap' }}>
              {BAR_WALL_THEMES.map(wt => {
                ...
                return (
                  <TouchableOpacity key={wt.id} onPress={() => requireAuth(() => setBarWallThemeId(wt.id))}
            ...
            <View>
              ...
              <View style={{ flexDirection: 'row', gap: 14, flexWrap: 'wrap' }}>
                {BAR_LIGHT_COLORS.map(lc => {
                  ...
                  return (
                    <TouchableOpacity key={lc.id} onPress={() => requireAuth(() => setBarLightColor(lc.color))}
                      style={{ alignItems: 'center', gap: 6 }}>
                      <View style={{
                        width: 46, height: 46, borderRadius: 23,
...
function HomeScreen() {
  const {
    ...
    setAuthModalVisible, barNeonColor,
  } = useAppStore();
  const { user } = useAuthStore();
  const styles = useStyles();
  const t      = TEXTS[lang];
    ...
    <View style={styles.home}>
      ...
      {barModeVisible && focusedItem == null && (headerH + atmosphereH) > 0 && (
        <View style={{
          ...
          backgroundColor: isDark ? C.surfaceHi : C.surface,
      ...
      {!user && (
        <TouchableOpacity onPress={() => setAuthModalVisible(true)}
          style={{
            marginHorizontal: 18, marginBottom: 6, marginTop: 2,
            backgroundColor: isDark ? 'rgba(255,184,48,0.10)' : 'rgba(184,120,0,0.08)',
            borderRadius: 12, borderWidth: 1,
            borderColor: isDark ? 'rgba(255,184,48,0.25)' : 'rgba(184,120,0,0.20)',
            paddingHorizontal: 14, paddingVertical: 10,
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 13, fontWeight: '700', color: isDark ? '#ffb830' : '#b87800' }}>
              {lang === 'ko' ? '로그인하면 나만의 바를 저장할 수 있어요' : 'Sign in to save your personal bar'}
            </Text>
            <Text style={{ fontSize: 11, color: C.textDim, marginTop: 2 }}>
              {lang === 'ko' ? '탭하여 로그인' : 'Tap to sign in'}
            </Text>
          </View>
          <Text style={{ fontSize: 13, color: isDark ? '#ffb830' : '#b87800', fontWeight: '700' }}>›</Text>
        </TouchableOpacity>
        ...
        {cabH > 0 && (
          <>
            ...
            <ShelfRow
              ...
              onDragEnd={onDragEnd}
            />
            {/* Wall-mounted CD player — between shelves, right wall */}
            <View style={{ position: 'absolute', right: -62, top: shelfPlanks[0] + 16 }}>
              <WallCDPlayer />
            </View>
            <CounterDecorations />
            <BarCounterDecor />
...
const GLASS_SPECS: Record<GlassType, GlassSpec> = {
  ...
  martini:  { GT: 255, GB: 415, WT: 80, WB:  6, label: 'Martini',  labelKo: '마티니' },
...
const GLASS_THUMB: Record<GlassType, { tGT: number; tGB: number; tWT: number; tWB: number }> = {
  ...
  martini:  { tGT: 18, tGB: 62, tWT: 25, tWB:  2 },
...
function MixSimulator({ selected, lang, onReset, onStageChange, onSaveRecipe }: {
  ...
  const isDark = useIsDark();
  const { selections, addPart, removePart, iceCount, setIceCount, iceType, setIceType, glassType, setGlassType } = useMixStore();
  const [stage,     setStage]     = useState<SimStage>('select');
  const [mixProg,   setMixProg]   = useState(0);
  ...
  const startMix = () => {
    if (mixProg >= 100) return;
    mixTimer.current = setInterval(() => {
      setMixProg(p => Math.min(100, p + 8));
    }, 80);
  };
  const stopMix = () => {
  ...
  useEffect(() => {
    if (mixProg >= 100) stopMix();
  }, [mixProg]);

  useEffect(() => () => stopMix(), []);

  const handleReset = () => {
    setStage('select'); setGlassType('rocks'); setIceCount(0); setMixProg(0);
    onReset();
  };
  ...
  return (
    <View style={{ position: 'relative', height: SIM_H }}>
      ...
      <Svg width={W} height={SIM_H} viewBox={`0 0 ${W} ${SIM_H}`}
        ...
        {/* Ice — type-based rendering */}
        {showGlass && iceType !== 'none' && (() => {
          const glH = GB - GT;
          if (iceType === 'large') {
            const iw = Math.round(Math.min(WB * 2 - 8, 52));
            const ih = Math.round(glH * 0.36);
            const iy = GB - ih - 5;
            const ix = CX - iw / 2;
            return (
              <>
                <Defs>
                  <LinearGradient id="iceLg" x1="0" y1="0" x2="1" y2="1">
                    ...
                    <Stop offset="0.4" stopColor="rgba(205,242,255,0.42)" />
                    <Stop offset="1"   stopColor="rgba(160,222,252,0.16)" />
                  </LinearGradient>
                </Defs>
                <SvgRect x={ix} y={iy} width={iw} height={ih} rx={5}
                  fill="url(#iceLg)" stroke="rgba(255,255,255,0.72)" strokeWidth={1.1} />
          ...
          if (iceType === 'sphere') {
            const r  = Math.round(Math.min(WB * 0.82, glH * 0.33));
            const cy = GB - r - 5;
            return (
              <>
                <Defs>
                  <RadialGradient id="iceSph" cx="0.35" cy="0.28" r="0.72" fx="0.35" fy="0.28">
                    ...
                    <Stop offset="0.22" stopColor="rgba(228,248,255,0.68)" />
                    <Stop offset="0.60" stopColor="rgba(185,232,254,0.30)" />
                    <Stop offset="1"    stopColor="rgba(140,204,242,0.10)" />
                  </RadialGradient>
                </Defs>
                <Circle cx={CX} cy={cy} r={r}
                  fill="url(#iceSph)" stroke="rgba(255,255,255,0.58)" strokeWidth={1.0} />
                <Path
              ...
              <>
                {pieces.map((p, i) => (
                  <SvgRect key={i}
                    ...
                    fill="rgba(218,244,255,0.58)" stroke="rgba(255,255,255,0.65)" strokeWidth={0.7}
            ...
            <>
              <Defs>
                <LinearGradient id="iceCb" x1="0" y1="0" x2="1" y2="1">
                  ...
                  <Stop offset="0.5" stopColor="rgba(208,240,255,0.40)" />
                  <Stop offset="1"   stopColor="rgba(170,222,252,0.15)" />
                </LinearGradient>
              </Defs>
              {cubeOffsets.slice(0, iceCount).map((c, i) => (
                <SvgRect key={i}
                  x={CX + c.dx} y={GB + c.dy} width={c.w} height={c.h} rx={2.5}
                  fill="url(#iceCb)" stroke="rgba(255,255,255,0.62)" strokeWidth={0.9}
                />
              ))}
            </>
          );
        ...
        {showGlass && (
          <>
            ...
            {glassType === 'martini' && (
              <>
                <Line x1={CX} y1={GB} x2={CX} y2={GB+13}
                  stroke="rgba(255,255,255,0.25)" strokeWidth={2.2} />
                <Line x1={CX-17} y1={GB+13} x2={CX+17} y2={GB+13}
                  stroke="rgba(255,255,255,0.30)" strokeWidth={2.2} strokeLinecap="round" />
              </>
            )}
      ...
      {stage === 'select' && (
        <View style={{
          ...
          backgroundColor: isDark ? '#0b0a17' : '#141220',
        }}>
          <Text style={{
            color: 'rgba(255,255,255,0.65)', fontSize: 13, fontWeight: '600',
            textAlign: 'center', marginBottom: 22, letterSpacing: 0.2,
          }}>
            ...
            {(['rocks', 'highball', 'martini', 'shot'] as GlassType[]).map(gt => {
              ...
              return (
                <TouchableOpacity key={gt}
                   onPress={() => setGlassType(gt)}
                  style={{
                    flex: 1, maxWidth: 84, alignItems: 'center', gap: 6,
          ...
          <TouchableOpacity
            ...
            style={{
              marginTop: 26, marginHorizontal: 18, paddingVertical: 13,
              borderRadius: 22, backgroundColor: 'rgba(255,255,255,0.11)',
      ...
      {stage === 'prep' && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'box-none' }}>
          <Text style={{
            position: 'absolute', top: GT - 78, left: 0, right: 0,
            textAlign: 'center', color: 'rgba(255,255,255,0.52)', fontSize: 12, fontWeight: '600', letterSpacing: 0.3,
          }}>
            {lang === 'ko' ? '얼음 종류를 선택하세요' : 'Choose your ice type'}
          </Text>

          {/* Ice type selector cards */}
          <ScrollView
            horizontal showsHorizontalScrollIndicator={false}
            style={{ position: 'absolute', top: GT - 52, left: 0, right: 0 }}
            contentContainerStyle={{ paddingHorizontal: 14, gap: 9, flexDirection: 'row', paddingVertical: 4 }}
          >
            {ICE_TYPES.map(it => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderColor: on ? 'rgba(200,238,255,0.80)' : 'rgba(255,255,255,0.14)',
                    backgroundColor: on ? 'rgba(180,228,255,0.10)' : 'rgba(255,255,255,0.03)',
                  }}
                >
                  {/* Mini SVG ice preview */}
                  ...
                  <Text style={{
                    ...
                    color: on ? 'rgba(200,238,255,0.96)' : 'rgba(255,255,255,0.45)',
                    textAlign: 'center',
                  }}>
                    {lang === 'ko' ? it.label : it.labelEn}
                  </Text>
                  <Text style={{
                    fontSize: 8, color: on ? 'rgba(200,238,255,0.65)' : 'rgba(255,255,255,0.22)',
                    textAlign: 'center', lineHeight: 11,
                  }} numberOfLines={2}>
                    {lang === 'ko' ? it.desc : it.descEn}
                  </Text>
                </TouchableOpacity>
              );
            })}
          ...
          {/* Cube count selector — only for cubed type */}
          {iceType === 'cubed' && (
            <View style={{
              position: 'absolute', top: GT + 56, left: 0, right: 0,
              ...
              <View style={{ alignItems: 'center', gap: 2 }}>
                <Text style={{ color: '#fff', fontSize: 20, fontWeight: '800', lineHeight: 24 }}>
                  {iceCount}
                </Text>
          ...
          <View style={{
            ...
            <TouchableOpacity onPress={() => setStage('select')} style={{ paddingVertical: 10, paddingHorizontal: 14 }}>
      ...
      {stage === 'mix' && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'box-none' }}>
          <Text style={{
            position: 'absolute', top: GT - 82, left: 0, right: 0,
            textAlign: 'center', fontSize: 14, fontWeight: '700',
            color: mixProg >= 100 ? '#a8f0c0' : 'rgba(255,255,255,0.62)',
          }}>
            {mixProg >= 100
              ? (lang === 'ko' ? '완성됐어요!' : 'Ready to serve!')
              : (lang === 'ko' ? '버튼을 꾹 눌러 믹스하세요' : 'Hold to mix')}
          </Text>
          <View style={{
            position: 'absolute', top: GT - 54, left: 34, right: 34,
            height: 5, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.10)',
          }}>
            <View style={{
              width: `${mixProg}%`, height: '100%', borderRadius: 3,
              backgroundColor: selected.length > 0 ? selected[0].color : '#8060ff',
            }} />
          </View>
          <View style={{ position: 'absolute', bottom: 22, left: 0, right: 0, alignItems: 'center' }}>
            {mixProg < 100 ? (
              <TouchableOpacity
                onPressIn={startMix} onPressOut={stopMix}
                style={{
              ...
              >
                <Svg width={26} height={36} viewBox="0 0 26 36">
                  <Path d="M 8 2 L 18 2 L 18 6 L 21 8 L 21 32 L 5 32 L 5 8 L 8 6 Z"
                    fill="rgba(255,255,255,0.42)" stroke="rgba(255,255,255,0.72)" strokeWidth={0.9} />
                  <Line x1={5} y1={10} x2={21} y2={10} stroke="rgba(255,255,255,0.38)" strokeWidth={0.8} />
                  <SvgRect x={8} y={14} width={10} height={3} rx={1.5} fill="rgba(255,255,255,0.22)" />
                  <SvgRect x={8} y={19} width={7} height={3} rx={1.5} fill="rgba(255,255,255,0.18)" />
                </Svg>
                <Text style={{ color: 'rgba(255,255,255,0.52)', fontSize: 9, fontWeight: '700' }}>
                  {lang === 'ko' ? '믹스' : 'MIX'}
                </Text>
              </TouchableOpacity>
            ) : (
      ...
      {stage === 'serve' && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, pointerEvents: 'box-none' }}>
          ...
          <View style={{
            position: 'absolute', top: GB + 14, left: 0, right: 0,
...
function RecipeSimPreview({ recipe, lang }: { recipe: Recipe; lang: 'en' | 'ko' }) {
  const isDark = useIsDark();
  const cx = 40, gt = 28, gb = 96, wt = 26, wb = 19;
  const liquidTop = gt + 18;
  const hw = (y: number) => wb + (wt - wb) * (gb - y) / (gb - gt);
  const hwLiquidTop = hw(liquidTop);
  const hwGb = hw(gb);
  return (
    <View style={{
      height: 136, backgroundColor: isDark ? '#0e0d1a' : '#16141f',
      alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
    }}>
      <View style={{
        position: 'absolute', top: 0, left: 0, right: 0, height: 68,
        backgroundColor: `${recipe.color}${isDark ? '38' : '30'}`,
      }} />
      <Svg width={80} height={104} viewBox="0 0 80 120">
        <Defs>
          <LinearGradient id={`rsp_${recipe.id}`} x1="0" y1="0" x2="0" y2="1">
            <Stop offset="0" stopColor={recipe.color} stopOpacity={0.95} />
            <Stop offset="1" stopColor={recipe.color} stopOpacity={0.62} />
          </LinearGradient>
        </Defs>
        <Path d={`M ${cx-wt} ${gt} L ${cx+wt} ${gt} L ${cx+wb} ${gb} L ${cx-wb} ${gb} Z`}
          fill="rgba(255,255,255,0.03)" stroke="rgba(255,255,255,0.30)" strokeWidth={1.5} />
        <Path d={`M ${cx-hwLiquidTop} ${liquidTop} L ${cx+hwLiquidTop} ${liquidTop} L ${cx+hwGb} ${gb} L ${cx-hwGb} ${gb} Z`}
          fill={`url(#rsp_${recipe.id})`} />
        <Line x1={cx-hwLiquidTop+3} y1={liquidTop} x2={cx+hwLiquidTop-3} y2={liquidTop}
          stroke="rgba(255,255,255,0.35)" strokeWidth={1.2} />
        <SvgRect x={cx-14} y={liquidTop+4} width={13} height={10} rx={2}
          fill="rgba(255,255,255,0.55)" stroke="rgba(255,255,255,0.70)" strokeWidth={0.6} />
        <SvgRect x={cx+3} y={liquidTop+10} width={11} height={9} rx={2}
          fill="rgba(255,255,255,0.45)" stroke="rgba(255,255,255,0.65)" strokeWidth={0.6} />
        <Line x1={cx-wt} y1={gt} x2={cx+wt} y2={gt}
          stroke="rgba(255,255,255,0.45)" strokeWidth={2} strokeLinecap="round" />
      </Svg>
      <Text style={{
        color: 'rgba(255,255,255,0.42)', fontSize: 9, fontWeight: '600',
        letterSpacing: 1.1, marginTop: 2,
      }}>
        {lang === 'ko' ? '시뮬레이션 미리보기' : 'SIMULATION PREVIEW'}
      </Text>
    </View>
  );
}
    ...
    >
      ...
      {expanded && (
        <View style={{ borderTopWidth: 1, borderTopColor: isDark ? '#1e1e2e' : '#e8e4de' }}>
          ...
          {/* Simulation preview — 시뮬레이션 이미지 */}
          <RecipeSimPreview recipe={recipe} lang={lang} />
          ...
          <View style={{ paddingHorizontal: 16, paddingBottom: 18, paddingTop: 14 }}>
            ...
            {ingredients.map((ing, i) => (
              <View key={i} style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 6 }}>
                <View style={{ width: 5, height: 5, borderRadius: 2.5, backgroundColor: recipe.color, marginRight: 10 }} />
                <Text style={{ fontSize: 13, color: isDark ? '#c0c0d0' : '#3a3830' }}>{ing}</Text>
              </View>
            ))}
...
function MixScreen() {
  const { lang, addJournalEntry, customRecipes, addCustomRecipe, toggleSavedRecipe, journalEntries, inventoryItems } = useAppStore();
  const t                            = TEXTS[lang];
  const C                            = useColors();
  const styles                       = useStyles();
  ...
  const handleSaveRecipe = (glass: GlassType, ice: number) => {
    ...
    // Calculate final ABV with dilution
    let totalParts = 0;
    let sumAbv = 0;
    selections.forEach(sel => {
      const ing = selected.find(x => x.id === sel.ingredientId);
    ...
    const dilution = 1 / (1 + ice * 0.08);
    const finalAbv = rawAbv * dilution;

    const id = `custom_${Date.now()}`;
    const primaryIng = selected.length > 0 ? selected[0] : null;
    const nameEn = primaryIng ? `${primaryIng.name.split(' ')[0]} Custom Mix` : 'My Custom Mix';
    const nameKo = primaryIng ? `${primaryIng.nameKo} 커스텀 믹스` : '나의 커스텀 믹스';
    ...
    toggleSavedRecipe(id); // Save immediately
    Alert.alert(
      lang === 'ko' ? '레시피 저장됨' : 'Recipe Saved',
      lang === 'ko' ? '커스텀 레시피가 보관함에 저장되었습니다!' : 'Your custom recipe has been saved to the Archive!'
    );
  };
...
const BAR_POSTS: BarPost[] = [
  { id:'bp1',  userName:'Ayoung Kim',    userNameKo:'김아영', initials:'AK', c1:'#ff9a9e', c2:'#f67280', followers:'13K',
    ...
    shelf0:['Gin','Tequila','Vodka'], shelf1:['Campari','Amaretto'],
    likes:342, cardH:275,
    neonColor:'#00e0ff', wallThemeId:'slate', lightColor:'#60d0ff' },
  { id:'bp2',  userName:'Jisoo Park',    userNameKo:'박지수', initials:'JP', c1:'#a18cd1', c2:'#7c4dff', followers:'8.2K',
    barName:'Whisky Shelf',    barNameKo:'위스키 셀프',
    shelf0:['Bourbon','Scotch','Cognac'], shelf1:['Amaretto','Campari'],
    likes:189, cardH:205,
    neonColor:'#9060ff', wallThemeId:'wood', lightColor:'#ffd060' },
  { id:'bp3',  userName:'Minho Lee',     userNameKo:'이민호', initials:'ML', c1:'#43c6ac', c2:'#1976d2', followers:'21K',
    ...
    shelf0:['Vodka','Tequila','Gin'], shelf1:['Campari','Bourbon'],
    likes:567, cardH:205,
    neonColor:'#00e880', wallThemeId:'forest', lightColor:'#60ff90' },
  { id:'bp4',  userName:'Sooyeon Choi',  userNameKo:'최수연', initials:'SC', c1:'#ffb347', c2:'#e64a19', followers:'5.7K',
    ...
    shelf0:['Gin','Campari','Vodka'], shelf1:['Tequila','Amaretto'],
    likes:98,  cardH:275,
    neonColor:'#ffb830', wallThemeId:'brick', lightColor:'#ff9820' },
  { id:'bp5',  userName:'Taehyun Yoon',  userNameKo:'윤태현', initials:'TY', c1:'#84fab0', c2:'#0f9b58', followers:'32K',
    ...
    shelf0:['Bourbon','Gin','Campari','Vodka'], shelf1:['Scotch','Cognac'],
    likes:891, cardH:275,
    neonColor:'#e040fb', wallThemeId:'wood', lightColor:'#ffd060' },
  { id:'bp6',  userName:'Jiwon Han',     userNameKo:'한지원', initials:'JH', c1:'#f093fb', c2:'#f5576c', followers:'4.1K',
    ...
    shelf0:['Cognac','Amaretto','Campari'], shelf1:['Gin','Vodka'],
    likes:215, cardH:205,
    neonColor:'#ff4060', wallThemeId:'brick', lightColor:'#ff80c0' },
  { id:'bp7',  userName:'Minseok Oh',    userNameKo:'오민석', initials:'MO', c1:'#4facfe', c2:'#00f2fe', followers:'17K',
    ...
    shelf0:['Scotch','Bourbon'], shelf1:['Cognac','Campari','Vodka'],
    likes:443, cardH:205,
    neonColor:'#00e0ff', wallThemeId:'slate', lightColor:'#60d0ff' },
  { id:'bp8',  userName:'Yura Song',     userNameKo:'송유라', initials:'YS', c1:'#fa709a', c2:'#fee140', followers:'9.8K',
    ...
    shelf0:['Tequila','Campari','Amaretto'], shelf1:['Vodka','Gin'],
    likes:312, cardH:275,
    neonColor:'#ffb830', wallThemeId:'forest', lightColor:'#ff9820' },
  { id:'bp9',  userName:'Dohyun Kang',   userNameKo:'강도현', initials:'DK', c1:'#e0c3fc', c2:'#8ec5fc', followers:'6.3K',
    ...
    shelf0:['Campari','Gin','Bourbon'], shelf1:['Amaretto','Scotch'],
    likes:478, cardH:275,
    neonColor:'#9060ff', wallThemeId:'slate', lightColor:'#c060ff' },
  { id:'bp10', userName:'Sohee Lim',     userNameKo:'임소희', initials:'SL', c1:'#ffecd2', c2:'#fcb69f', followers:'11K',
    ...
    shelf0:['Campari','Cognac'], shelf1:['Amaretto','Tequila','Vodka'],
    likes:203, cardH:205,
    neonColor:'#ff4060', wallThemeId:'brick', lightColor:'#ffd060' },
  { id:'bp11', userName:'Junho Bae',     userNameKo:'배준호', initials:'JB', c1:'#a1c4fd', c2:'#c2e9fb', followers:'28K',
    barName:'Rum Shelf',       barNameKo:'럼 컬렉션',
    shelf0:['Vodka','Gin','Tequila'], shelf1:['Campari','Bourbon'],
    likes:763, cardH:205,
    neonColor:'#00e0ff', wallThemeId:'wood', lightColor:'#60d0ff' },
  { id:'bp12', userName:'Eunbi Cho',     userNameKo:'조은비', initials:'EC', c1:'#fd7043', c2:'#ff8a65', followers:'3.9K',
    ...
    shelf0:['Tequila','Campari','Cognac'], shelf1:['Bourbon','Vodka'],
    likes:91,  cardH:275,
    neonColor:'#ff4060', wallThemeId:'forest', lightColor:'#ff9820' },
  { id:'bp13', userName:'Kyungmin Shin', userNameKo:'신경민', initials:'KS', c1:'#667eea', c2:'#764ba2', followers:'19K',
    ...
    shelf0:['Cognac','Scotch','Bourbon'], shelf1:['Amaretto','Tequila'],
    likes:534, cardH:275,
    neonColor:'#9060ff', wallThemeId:'wood', lightColor:'#ffd060' },
  { id:'bp14', userName:'Nayeon Jung',   userNameKo:'정나연', initials:'NJ', c1:'#f7971e', c2:'#ffd200', followers:'7.4K',
    ...
    shelf0:['Gin','Vodka','Tequila'], shelf1:['Campari','Cognac'],
    likes:289, cardH:205,
    neonColor:'#ffb830', wallThemeId:'slate', lightColor:'#60ff90' },
  { id:'bp15', userName:'Woojin Park',   userNameKo:'박우진', initials:'WP', c1:'#11998e', c2:'#38ef7d', followers:'14K',
    ...
    shelf0:['Vodka','Gin','Bourbon'], shelf1:['Tequila','Campari'],
    likes:621, cardH:205,
    neonColor:'#00e880', wallThemeId:'wood', lightColor:'#60ff90' },
  { id:'bp16', userName:'Hyeri Moon',    userNameKo:'문혜리', initials:'HM', c1:'#ee9ca7', c2:'#ffdde1', followers:'5.1K',
    ...
    shelf0:['Amaretto','Cognac','Campari'], shelf1:['Gin','Scotch'],
    likes:157, cardH:275,
    neonColor:'#e040fb', wallThemeId:'brick', lightColor:'#ff80c0' },
];
...
function VisitorShelfRow({ plankY, bottles }: { plankY: number; bottles: string[] }) {
  const isDark  = useIsDark();
  const shelfW  = SCREEN_W - SHELF_MX * 2;
    ...
    <View style={{ position: 'absolute', top: plankY - BOTTLE_H, left: SHELF_MX, right: SHELF_MX, height: BOTTLE_H + PLANK_H + 6 }}>
      ...
      >
        {bottles.map((name, i) => {
          ...
          return (
            <View key={`v_${name}_${i}`} style={{ width: SLOT_W, height: BOTTLE_H }}>
              <ExpoImage
...
function VisitorBarModal({ post, visible, onClose }: {
  ...
  useEffect(() => { if (!visible) setCabH(0); }, [visible]);
  ...
  const SHEET_H = Math.round(SH * 0.86);
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Svg width={36} height={36} viewBox="0 0 36 36">
              <Defs>
                <LinearGradient id="vbAvatar" x1="0" y1="0" x2="0" y2="1">
                  <Stop offset="0" stopColor={post.c1} />
                  <Stop offset="1" stopColor={post.c2} />
                </LinearGradient>
              </Defs>
              <Circle cx="18" cy="18" r="18" fill="url(#vbAvatar)" />
              <SvgText x="18" y="23" textAnchor="middle" fontSize="12" fontWeight="700" fill="rgba(255,255,255,0.92)">
                {post.initials}
              </SvgText>
            </Svg>
            ...
            >
              <Svg width={16} height={15} viewBox="0 0 16 15">
                <Path d="M8 13.5 L2 8 Q-0.5 5 2.5 2 Q5.5 -0.5 8 3 Q10.5 -0.5 13.5 2 Q16.5 5 14 8 Z"
                  ...
                  strokeWidth={liked ? 0 : 1.4}
                />
              </Svg>
              <Text style={{ color: liked ? '#e0055e' : C.textDim, fontSize: 12, fontWeight: '600' }}>
          ...
          >
            <VisitorBarWall barName={displayName} neonColor={post.neonColor} wallThemeId={post.wallThemeId} />

            {cabH > 0 && (
              <>
                <VisitorPendantLights cabH={cabH} lightColor={post.lightColor} />
                <VisitorShelfRow plankY={shelfPlanks[0]} bottles={post.shelf0} />
                <VisitorShelfRow plankY={shelfPlanks[1]} bottles={post.shelf1} />
              </>
            )}
...
function MiniBarPreview({ shelf0, shelf1, height, wallThemeId, neonColor, lightColor }: {
  ...
  const wallTheme = BAR_WALL_THEMES.find(t => t.id === wallThemeId) ?? BAR_WALL_THEMES[0];
  const wallBg    = isDark ? wallTheme.darkBg : wallTheme.lightBg;
  const counterBg = isDark ? '#4a3218' : '#b07840';
  const counterSdw= isDark ? '#0e0907' : '#7a5020';
  const ambientGlow = isDark ? `${lightColor}1f` : `${lightColor}2e`;
  const reflectClr  = isDark ? `${lightColor}1a` : `${lightColor}26`;
  const signGlow    = isDark ? `${neonColor}59` : `${neonColor}3d`;

  // Bar counter sits at 84% from top
  const counterY  = Math.round(height * 0.84);
  const counterH  = 7;

  // Hero bottles (shelf0): tall, from top to counter
  const heroH = counterY - Math.round(height * 0.05);
  const heroW = Math.round(heroH * 0.42);

  // Background accent bottles (shelf1): ~55% of hero, faded
  const bgH = Math.round(heroH * 0.55);
  const bgW = Math.round(bgH * 0.42);

  // Only show up to 3 hero bottles to keep them large
  const heroBottles = shelf0.slice(0, 3);
  const bgBottles   = shelf1.slice(0, 2);

  return (
    <View style={{ width: '100%', height, backgroundColor: wallBg, overflow: 'hidden', borderTopLeftRadius: 12, borderTopRightRadius: 12 }}>
      {/* Neon sign glow accent near top */}
      <View style={{
        position: 'absolute',
        top: -Math.round(height * 0.08), left: '30%', right: '30%',
        height: Math.round(height * 0.3),
        backgroundColor: signGlow, borderRadius: height,
      }} />

      {/* Warm ambient blob behind bottles */}
      <View style={{
        position: 'absolute',
        top: Math.round(height * 0.15), left: '20%', right: '20%',
        height: Math.round(height * 0.55),
        backgroundColor: ambientGlow, borderRadius: height,
      }} />

      {/* Subtle wall texture lines */}
      {[0.18, 0.46, 0.70].map(f => (
        <View key={f} style={{
      ...
      {/* Background bottles (shelf1) — faded, smaller, tucked behind */}
      {bgBottles.length > 0 && (
        <View style={{
          position: 'absolute',
          top: counterY - bgH, left: 0, right: 0, height: bgH,
          flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'flex-end',
          opacity: 0.38, paddingHorizontal: 8,
        }}>
          {bgBottles.map((name, i) => (
            <ExpoImage key={`bg_${i}`}
              source={{ uri: `https://www.thecocktaildb.com/images/ingredients/${name}-Medium.png` }}
              style={{ width: bgW, height: bgH }} contentFit="contain" />
          ))}
        </View>
      )}

      {/* Hero bottles (shelf0) — large, prominent */}
      <View style={{
        position: 'absolute',
        top: counterY - heroH, left: 0, right: 0, height: heroH,
        flexDirection: 'row', justifyContent: 'space-evenly', alignItems: 'flex-end',
        paddingHorizontal: 4,
      }}>
        {heroBottles.map((name, i) => (
          <ExpoImage key={`hero_${i}`}
            source={{ uri: `https://www.thecocktaildb.com/images/ingredients/${name}-Medium.png` }}
            style={{ width: heroW, height: heroH }} contentFit="contain" />
        ))}
      </View>

      {/* Bar counter surface */}
      <View style={{ position: 'absolute', top: counterY, left: 0, right: 0, height: counterH, backgroundColor: counterBg }} />
      <View style={{ position: 'absolute', top: counterY + counterH, left: 0, right: 0, height: 3, backgroundColor: counterSdw }} />

      {/* Counter reflection highlight */}
      <View style={{ position: 'absolute', top: counterY + 1, left: 0, right: 0, height: 1, backgroundColor: reflectClr }} />

      {/* Below counter fill */}
      <View style={{ position: 'absolute', top: counterY + counterH + 3, left: 0, right: 0, bottom: 0, backgroundColor: isDark ? '#0d0906' : '#8a5e2a' }} />
    </View>
  );
}
...
// ─────────────────────────────────────────────────────────────────
type ArchiveFilter = 'all' | 'bars' | 'spirits' | 'recipes';
...
function ProfileScreen() {
  const { lang, toggleLang, theme, toggleTheme,
          savedBarIds, savedSpiritIds, savedRecipeIds, inventoryItems,
          setAuthModalVisible } = useAppStore();
  const { user } = useAuthStore();
  const C       = useColors();
  const styles  = useStyles();
  const t       = TEXTS[lang];
  ...
  const totalCount = savedBars.length + savedSpirits.length + savedRecipes.length;

  const FILTERS: Array<{ id: ArchiveFilter; label: string; labelKo: string; count: number }> = [
    { id:'all',     label:'All',     labelKo:'전체',   count: totalCount },
    { id:'bars',    label:'Bars',    labelKo:'바',     count: savedBars.length },
  ...
  const showBars    = filter === 'all' || filter === 'bars';
  const showSpirits = filter === 'all' || filter === 'spirits';
  const showRecipes = filter === 'all' || filter === 'recipes';
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: C.bg }}>
      {/* Header */}
      <View style={{ flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: 20, paddingTop: 14, paddingBottom: 12 }}>
        <View style={{ flex: 1 }}>
          <Text style={{ fontSize: 12, color: C.textDim, marginTop: 2 }}>
            {user
              ? (profileView === 'genome'
                  ? (lang === 'ko' ? `${inventoryItems.filter(i=>i.myRating).length}개 평가 기록 반영` : `${inventoryItems.filter(i=>i.myRating).length} rated spirits`)
                  : (lang === 'ko' ? `총 ${totalCount}개 저장됨` : `${totalCount} items saved`))
              : (lang === 'ko' ? '비회원으로 이용 중' : 'Browsing as guest')}
          </Text>
        </View>
        <View style={{ flexDirection: 'row', gap: 8 }}>
          {/* 비회원 시 로그인 버튼 */}
          {!user && (
            <TouchableOpacity onPress={() => setAuthModalVisible(true)}
              style={{ height: 36, paddingHorizontal: 14, borderRadius: 10, backgroundColor: C.primary, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 6 }}>
              <Svg width={14} height={14} viewBox="0 0 24 24" fill="none">
      ...
      {showSettings && (
        <View style={{ marginHorizontal: 20, marginBottom: 10, backgroundColor: C.surfaceHi, borderRadius: 14, borderWidth: 1, borderColor: C.border, padding: 14, gap: 0 }}>
      ...
      {profileView === 'genome' ? (
        ...
        <>
          {/* Filter chips */}
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ maxHeight: 40 }} contentContainerStyle={{ paddingHorizontal: 20, gap: 8, flexDirection: 'row' }}>
            {FILTERS.map(f => {
              const active = filter === f.id;
              return (
                <TouchableOpacity key={f.id} onPress={() => setFilter(f.id)}
          ...
          <>
            ...
            {showBars && (
              <>
                {(filter === 'all') && (
                  <Text style={{ fontSize: 11, fontWeight: '800', color: C.textDim, letterSpacing: 1.1, textTransform: 'uppercase', marginBottom: 10 }}>
                    {lang === 'ko' ? `저장한 바 (${savedBars.length})` : `Saved Bars (${savedBars.length})`}
                  </Text>
                )}
                {savedBars.length === 0
                  ? <ArchiveEmptyState icon="🍸" label={lang === 'ko' ? '저장한 바가 없어요' : 'No saved bars'} />
                  : (
                    <View style={{ flexDirection: 'row', gap: 8 }}>
                      <View style={{ flex: 1 }}>
                        {savedBars.filter((_, i) => i % 2 === 0).map(p => <ArchiveBarCard key={p.id} post={p} lang={lang} onPress={setVisitPost} />)}
                      </View>
                      <View style={{ flex: 1 }}>
                        {savedBars.filter((_, i) => i % 2 === 1).map(p => <ArchiveBarCard key={p.id} post={p} lang={lang} onPress={setVisitPost} />)}
                      </View>
                    </View>
                  )
                }
            ...
            {showSpirits && (
              <>
                {(filter === 'all') && (
                  <Text style={{ fontSize: 11, fontWeight: '800', color: C.textDim, letterSpacing: 1.1, textTransform: 'uppercase', marginBottom: 10, marginTop: showBars && savedBars.length > 0 ? 16 : 0 }}>
            ...
            {showRecipes && (
              <>
                {(filter === 'all') && (
                  <Text style={{ fontSize: 11, fontWeight: '800', color: C.textDim, letterSpacing: 1.1, textTransform: 'uppercase', marginBottom: 10, marginTop: (showBars || showSpirits) && (savedBars.length + savedSpirits.length) > 0 ? 16 : 0 }}>
...
// ─────────────────────────────────────────────────────────────────
// AUTH MODAL  (전역 팝업 — ProfileScreen 로그인 버튼 / 기능 잠금에서 호출)
// ─────────────────────────────────────────────────────────────────
function AuthModal() {
  const { authModalVisible, setAuthModalVisible } = useAppStore();
  const { user } = useAuthStore();

  // 로그인 성공 시 자동 닫기
  useEffect(() => {
    if (user && authModalVisible) setAuthModalVisible(false);
  }, [user, authModalVisible, setAuthModalVisible]);
  ...
  return (
    ...
    >
      <AuthScreen onClose={() => setAuthModalVisible(false)} />
    </Modal>
  );
}
